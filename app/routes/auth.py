...
# 🆕 Реєстрація користувача (з підтримкою secret_code)
...
async def register(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # ✅ Перевіряємо, чи є `secret_code` в отриманих даних
...
async def request_password_reset(data: PasswordResetRequest, db: AsyncSession = Depends(get_db)):
    ...
    await redis_client.setex(
        f"password-reset:{user.email}", settings.RESET_TOKEN_EXPIRE_MINUTES * 60, token
    )
...
# 🆕 Отримання всіх користувачів (НЕЗАХИЩЕНИЙ маршрут)
